"""Train a single-agent PPO baseline for the travel task."""
...
from drone_rl.travel_ppo import TravelPPOConfig, show_model_performance, train_travel_agent


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for travel-stage training."""
    parser = argparse.ArgumentParser(description="Train a PPO travel baseline.")
    ...
    parser.add_argument("--reward-threshold", type=float, default=45.0, help="Early-stop reward threshold.")
...
def main() -> None:
    """Run travel PPO training and optionally show policy rollout performance."""
    args = parse_args()
    ...
    run_dir = train_travel_agent(config=config)
