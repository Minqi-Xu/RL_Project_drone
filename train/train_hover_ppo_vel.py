"""Train a single-agent PPO baseline for the hover task using VEL actions."""
...
from drone_rl.hover_ppo_vel import HoverVelPPOConfig, show_vel_model_performance, train_hover_vel_agent


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a PPO hover baseline with ActionType.VEL.")
...
def main() -> None:
    ...
    run_dir = train_hover_vel_agent(config=config)
    print("[INFO] VEL hover training finished. Run directory:", run_dir)
    ...
    if args.show_performance:
        ...
        print("[INFO] Showing VEL hover policy performance from:", model_path)
