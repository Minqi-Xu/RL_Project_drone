"""Train a single-agent PPO baseline for the hover task."""
...
from drone_rl.hover_ppo import HoverPPOConfig, show_model_performance, train_hover_agent


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a PPO hover baseline.")
...
def main() -> None:
    args = parse_args()
    # Override only the settings we expose at the command line and keep the rest in the shared config.
    ...
    run_dir = train_hover_agent(config=config)
