"""Shared PPO travel training utilities built on top of gym-pybullet-drones."""
...
class TravelPPOConfig:
    """Configuration for the single-agent travel baseline."""
...
def build_run_dir(base_dir: Path) -> Path:
    ...
    run_dir = base_dir / f"travel_ppo_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    run_dir.mkdir(parents=True, exist_ok=True)
    return run_dir


def get_obs_type(config: TravelPPOConfig) -> ObservationType:
    return ObservationType(config.observation)


def get_action_type(config: TravelPPOConfig) -> ActionType:
    return ActionType(config.action)


def make_train_env(config: TravelPPOConfig):
...
def make_eval_env(config: TravelPPOConfig, gui: bool = False):
    # Evaluation uses a plain ProjectTravelAviary instance so we can optionally render rollouts.
    return ProjectTravelAviary(
        gui=gui,
        record=False,
...
def build_model(train_env, run_dir: Path, config: TravelPPOConfig) -> PPO:
...
def build_eval_callback(eval_env, run_dir: Path, config: TravelPPOConfig) -> EvalCallback:
    # Stop training early once the travel reward is consistently good enough.
...
def train_travel_agent(config: TravelPPOConfig, results_dir: Path | None = None) -> Path:
    """Train a PPO policy for the travel task and return the run directory."""
...
def evaluate_saved_model(
    model_path: Path,
    config: TravelPPOConfig,
    n_eval_episodes: int | None = None,
) -> tuple[float, float]:
    """Compute aggregate evaluation metrics for a saved travel policy."""
...
def show_model_performance(
    ...
    """Run one rendered rollout for traveling and stop at the first episode end."""
    model = PPO.load(str(model_path))
